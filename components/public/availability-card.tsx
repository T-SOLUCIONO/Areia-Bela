...
export function AvailabilityCard({ className }: Props) {
  ...
  return (
    <aside className={cn('w-full rounded-[28px] bg-white/95 p-5 shadow-[0_24px_80px_rgba(15,23,42,0.18)] ring-1 ring-black/5 sm:p-6', className)}>
            ...
            <div className="rounded-xl border border-slate-200 bg-white px-3 py-3 transition hover:border-slate-300 hover:bg-slate-50">
              <div className="flex items-center justify-between gap-3">
                <div>
                  ...
                  <div className="mt-0.5 text-sm text-slate-800">{checkIn ? format(checkIn, 'MMM d') : copy.addDate}</div>
                </div>
            ...
            <div className="rounded-xl border border-slate-200 bg-white px-3 py-3 transition hover:border-slate-300 hover:bg-slate-50">
              <div className="flex items-center justify-between gap-3">
                <div>
                  ...
                  <div className="mt-0.5 text-sm text-slate-800">{checkOut ? format(checkOut, 'MMM d') : copy.addDate}</div>
                </div>
                <CalendarDays className="h-4 w-4 text-slate-500" />
              </div>
            </div>
      ...
      <Popover open={guestsOpen} onOpenChange={setGuestsOpen}>
        <PopoverTrigger asChild>
          <button type="button" className="mt-3 w-full rounded-[18px] border border-slate-200 bg-white px-4 py-3 text-left transition hover:border-slate-300 hover:bg-slate-50">
            <div className="flex items-center justify-between gap-3">
              <div>
              ...
              <ChevronDown className={cn('h-5 w-5 text-slate-500 transition-transform', guestsOpen && 'rotate-180')} />
            </div>
          </button>
        </PopoverTrigger>
        <PopoverContent className="w-[min(100vw-1rem,440px)] rounded-[22px] border-slate-200 p-0 shadow-[0_24px_70px_rgba(15,23,42,0.14)]" align="start" sideOffset={10}>
          <div className="space-y-5 px-5 py-5">
            {guestRows.map((item) => (
              <div key={item.key} className="flex items-center justify-between gap-4">
                <div>
                  <p className="text-[22px] font-semibold leading-tight text-slate-900">{item.title}</p>
                  <p className="mt-1 text-[16px] leading-tight text-slate-500">{item.description}</p>
                </div>
                <div className="flex items-center gap-4">
                  ...
                  <span className="min-w-6 text-center text-[24px] text-slate-900">{guests[item.key]}</span>
                  <button
                    type="button"
